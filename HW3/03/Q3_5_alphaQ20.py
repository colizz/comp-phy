...
if __name__ == '__main__':
    ...
    Et_acc = [[Et[i][j] for j in range(len(Et[0]))] for i in range(len(Et))]
    for j in range(8):
    ...
    for i in range(8): ## 对k=1...8 做平均能量图
        plt.plot([t_list[i] /(2*pi/ome[0]) for i in range(len(t_list))], [Et_acc[m][i] for m in range(len(Et))], label='%d-th'%(i+1))
    plt.xlim([0, TN])
    plt.legend()
    plt.xlabel(r'$T\,/\,(2\pi/\omega_1)$')
